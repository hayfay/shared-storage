...
class Application(Frame):
    ...
    def print_solution(self, solution):
            ...
            for i in range(len(table)):
                for j in range(len(table[1])):
                    if i == 0:
                        ...
                        if i == solution['rk1'][l][0] + 1 and j == solution['rk1'][l][1] + 1:
                            Label(frame, text=str(table[i][j]), bg='lightblue').grid(row=i, column=j, padx=1, pady=1)
                        else:
            ...
            for i in range(len(table)):
                for j in range(len(table[1])):
                    if i == 0:
                        ...
                        if i == solution['rk2'][l][0] + 1 and j == solution['rk2'][l][1] + 1:
                            Label(frame, text=str(table[i][j]), bg='lightblue').grid(row=i, column=j, padx=1, pady=1)
                        else:
                            Label(frame, text=str(table[i][j])).grid(row=i, column=j, padx=1, pady=1)
            l += 1
        if solution['error2'] != 'None':
            ...
            return
        Label(self,
              text="В строке коэффициентов целевой функции нет положительных элементов - оптимальное решение найдено.").pack()
        # Печать ответа
