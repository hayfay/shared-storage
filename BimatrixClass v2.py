    ...
    for row in matrix:
        ...
        for elem in row:
            if (i, j) in pareto and (i, j) in nash:
                print("{2}({0: 3d}/{1: 3d})".format(elem[0], elem[1], Fore.BLUE) , end=' ')
                print(Style.RESET_ALL, end='')
            elif (i, j) in pareto:
...
print("Устойчивые по Нэшу выделены", Fore.YELLOW, "желтым", Style.RESET_ALL)
print("Эффективные по Парето и устойчивые по Нэшу одновременно выделены", Fore.BLUE, "голубым", Style.RESET_ALL)
